...
const formularioSchema = z.object({
  ...
  enviadosA: z.array(z.string()).min(1, "Debes seleccionar al menos un destinatario"),
});
...
export default function FormulariosDashboard() {
  ...
  const [invitados, setInvitados] = useState<{ _id: string; nombre: string; telefono: string }[]>([]);
  const [preguntas, setPreguntas] = useState([]);
  const [modalAbierto, setModalAbierto] = useState(false);
  const [formularioEditando, setFormularioEditando] = useState<Formulario | null>(null);

  const { formularios, loading, error, crearFormulario, eliminarFormulario, editarFormulario } = useFormularios(
    bodaId || "",
    token || ""
  );
  ...
  const [modoEnvio, setModoEnvio] = useState<"invitados" | "lista">("invitados");
  const [listasDifusion, setListasDifusion] = useState<{ _id: string; nombre: string; invitados: any[] }[]>([]); // array con listas
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const invitadosRes = await fetch(`http://localhost:4000/api/guests/invitados/${user.bodaId}?limit=100`, {
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="border p-6 rounded-lg shadow-lg space-y-4">
        <div>
          <label className="block font-semibold mb-2">📌 Nombre del formulario:</label>
          <Controller name="nombre" control={control} render={({ field }) => (
            <input {...field} className="w-full p-2 border rounded" placeholder="Ej: Confirmación de asistencia" />
          )} />
        </div>
  ...
  {modoEnvio === "invitados" ? (
    ...
    <>
      ...
      />
        ...
        <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded">✅ Crear Formulario</button>
      </form>

      {/* 📜 Lista de Formularios */}
      <h3 className="text-xl font-semibold mt-6 mb-2">📜 Formularios Creados:</h3>
      {loading ? (
        <p>Cargando...</p>
      ) : error ? (
        ...
        formularios.map((form) => (
         <div key={form._id} className="border p-4 rounded-lg flex justify-between items-center mb-1.5">
            <p className="font-bold">{form.nombre}</p>
            <div className="flex gap-2">
              <button onClick={() => eliminarFormulario(form._id)} className="bg-red-500 text-white px-3 py-1 rounded">🗑️</button>
              <button onClick={() => abrirModalEdicion(form)} className="bg-yellow-500 text-white px-3 py-1 rounded">✏️</button>
            </div>
          </div>
        ))
      )}
