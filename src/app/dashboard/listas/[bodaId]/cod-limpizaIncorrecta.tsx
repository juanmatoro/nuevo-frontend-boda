...
export default function ListasDifusionDashboard() {
  ...
  const { control, handleSubmit, reset, setValue } = useForm<ListaDifusionForm>({
...
const abrirEdicion = (lista: { _id: string; nombre: string; invitados: { _id: string }[] }) => {
    setEditandoLista(lista._id);
    setNombreEditado(lista.nombre);
    setInvitadosEditados(lista.invitados.map((i) => i._id));
};
  ...
  const guardarEdicion = async () => {
    ...
    try {
      await editarLista(editandoLista, { nombre: nombreEditado, invitados: invitadosEditados });
      setEditandoLista(null);

      // ✅ Limpiar los campos después de guardar los cambios
      setNombreEditado("");
      setInvitadosEditados([]);
    } catch (error) {
              ...
              {editandoLista === lista._id ? (
                ...
                <div className="w-full">
                  ...
                  <Select
                    ...
                    isMulti
                    value={invitados
                      .filter((i) => invitadosEditados.includes(i._id))
                      .map((i) => ({ value: i._id, label: `${i.nombre} (${i.telefono})` }))}
                    onChange={(selected) =>
                      setInvitadosEditados(selected.map((s) => s.value))
                    }
                    className="mb-2"
                  />
