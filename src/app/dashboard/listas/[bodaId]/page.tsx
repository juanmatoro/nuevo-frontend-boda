...
export default function ListasDifusionDashboard() {
      ...
      axiosInstance
        ...
        .then((res) => {
          if (Array.isArray(res.data.invitados)) {
            ...
            console.error(
              "❌ API no devolvió un array de invitados:",
              res.data
            );
            setInvitados([]);
          }
  ...
  const abrirEdicion = (lista: BroadcastList) => {
    ...
    setValue(
      "invitados",
      lista.invitados.map((i: Invitado) => i._id) as string[]
    );
  };
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      await editarLista(editandoLista, {
        nombre: data.nombre,
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        value={invitados
                          .filter((i) =>
                            ((field.value as string[]) || []).includes(i._id)
                          )
                          .map((i) => ({
                            value: i._id,
                            label: `${i.nombre} (${i.telefono})`,
